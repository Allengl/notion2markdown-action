...
// if metas_excluded is a string, split it by comma and trim the spaces
if (typeof metas_excluded === "string" && metas_excluded.trim().length > 0) {
  // if metas_excluded is a string, split it by comma and trim the spaces
  metas_excluded = metas_excluded.split(",").map((key) => key.trim());
}
