...
import {
  ...
  batchMigrateNotionImages,
} from "./migrateNotionImage.js";
...
async function sync() {
  ...
  for (let i = 0; i < localPostFileList.length; i++) {
    ...
    var localProp = loadPropertiesAndContentFromMarkdownFile(
      path.join(config.output_dir.post, localFilename)
    );
    if (!localProp) {
      continue;
    }
  ...
  if (config?.last_sync_datetime && config.last_sync_datetime !== null) {
    ...
    notionPagePropList = notionPagePropList.filter(
      (prop) =>
        ...
        moment(prop.last_edited_time) > moment(config.last_sync_datetime)
    );
  }
